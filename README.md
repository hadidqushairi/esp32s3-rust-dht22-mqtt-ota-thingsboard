...
## ALERT⚠️
make sure you have 3 code main.rs
- main code (include: sensor reading + mqtt thingsboard + wifi + anything else)
- OTA update code
- main code for update
...
use esp_idf_svc::{eventloop::EspSystemEventLoop, nvs::EspDefaultNvsPartition, wifi::{AuthMethod, BlockingWifi, ClientConfiguration, Configuration, EspWifi}};
...
// Helper function dari teman Anda untuk konversi waktu delay
#[inline(always)]
fn ms_to_ticks(ms: u32) -> u32 {
    (ms as u64 * configTICK_RATE_HZ as u64 / 1000) as u32
}
    ...
    fn publish(&self, topic: &str, data: &str, qos: i32) -> Result<()> {
        unsafe {
            ...
            let msg_id = esp_mqtt_client_publish(
                ...
                topic_cstr.as_ptr(),
                // --- PERBAIKAN FINAL ---
                // data.as_ptr() sudah mengembalikan *const u8, tidak perlu cast
                data.as_ptr(),
                data.len() as i32,
                qos,
                0,
            ...
            if msg_id < 0 {
                Err(anyhow::anyhow!("Gagal mempublikasikan pesan, kode error: {}", msg_id))
            } else {
...
fn main() {
    esp_idf_sys::link_patches(); 
    esp_idf_svc::log::EspLogger::initialize_default();
    info!("🚀 MEMULAI PROGRAM ESP32S3 DHT22 + WiFi + MQTT (Metode Low-Level)");
    ...
    match connect_wifi(&mut wifi) {
        Ok(_) => info!("✅ Koneksi WiFi Berhasil."),
        Err(e) => {
            error!("❌ Gagal terhubung ke WiFi: {:?}", e);
            return;
        }
    };
...
// Import untuk macro format di no_std
use alloc::format;

// Deklarasi bahwa kita akan menggunakan alokator memori
extern crate alloc; 

// TAMBAHAN UNTUK RTC DAN TIMESTAMP: Import untuk SNTP (no_std compatible)
use esp_idf_svc::sntp::{EspSntp, SyncMode, SntpConf};

// Import tambahan untuk status sync dan konversi waktu
use esp_idf_sys::{sntp_get_sync_status, sntp_sync_status_t_SNTP_SYNC_STATUS_COMPLETED, localtime_r, tm};
...
// Fungsi helper untuk konversi timestamp Unix (detik) ke string human-readable (YYYY-MM-DD HH:MM:SS)
fn timestamp_to_string(timestamp_sec: u64) -> String {
    let mut tm_struct: tm = unsafe { core::mem::zeroed() };
    unsafe {
        localtime_r(&(timestamp_sec as i64), &mut tm_struct);
    }
    format!(
        "{:04}-{:02}-{:02} {:02}:{:02}:{:02}",
...
fn main() {
    ...
    // TAMBAHAN UNTUK RTC: Sinkron waktu dengan SNTP untuk mendapatkan timestamp akurat dari RTC software
    info!("Menghubungkan ke server NTP untuk sinkron waktu...");
    let sntp_conf = SntpConf {
        sync_mode: SyncMode::Immediate,
        servers: ["pool.ntp.org"],
        ..Default::default()
    };
    let _sntp = match EspSntp::new(&sntp_conf) {
        Ok(sntp) => {
            info!("✅ Mulai sinkron waktu dengan NTP. Menunggu hingga selesai...");

            // Tambahkan loop tunggu hingga sync selesai (dengan timeout 30 detik)
            let max_retries = 300; // 30 detik (300 * 100ms)
            let mut retry = 0;
            unsafe {
                loop {
                    if sntp_get_sync_status() == sntp_sync_status_t_SNTP_SYNC_STATUS_COMPLETED {
                        info!("✅ Sinkron waktu SNTP selesai.");
                        break;
                    }
                    if retry >= max_retries {
                        error!("❌ Timeout menunggu sinkron SNTP setelah 30 detik.");
                        return;
                    }
                    retry += 1;
                    vTaskDelay(ms_to_ticks(100)); // Delay 100ms
                }
            }
    ...
    loop {
        ...
        for attempt in 1..=3 {  // Retry 3 kali jika gagal
            info!("Mencoba pembacaan DHT22 attempt {}...", attempt);
            match dht22.read() {
                Ok(r) => {
                    reading = Some(r);
                    break;
                ...
                Err(e) => {
                    error!("❌ Gagal membaca DHT22 pada attempt {}: {:?}", attempt, e);
                    unsafe { vTaskDelay(ms_to_ticks(2000)); }  // Delay 2 detik sebelum retry
                }
            }
        }
